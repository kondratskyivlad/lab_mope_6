...
while not correct_input:
    try:
        m = int(input(Fore.GREEN + "Введіть кількість повторень: "))
        p = float(input(Fore.GREEN + "Введіть довірчу імовірність: "))
...
class Critical_values:
    ...
    @staticmethod
    def get_student_value(f3, significance, scipy=None):
        from _pydecimal import Decimal
    ...
    for column in range(number_x + 1):
        ...
        for row in range(N):
            if column == 0:
                ...
                t_practice += average_y[row] * in_matrix[row][column - 1]
...
for i in range(len(matrix_x)):
    if i < 8:
        x_1 = x1_min if in_matrix[i][0] == -1 else x1_max
        x_2 = x2_min if in_matrix[i][1] == -1 else x2_max
        x_3 = x3_min if in_matrix[i][2] == -1 else x3_max
    else:
        x_lst = x(in_matrix[i][0], in_matrix[i][1], in_matrix[i][2])
        x_1, x_2, x_3 = x_lst
...
while not adequacy:
    ...
    beta = solve(values_arr, known)
    print(Fore.BLUE + "\tОтримане рівняння регресії")
    print(Style.RESET_ALL)
    ...
    while not homogeneity:
        print('\t')
        print(Fore.BLUE + "-" * 70 + "Матриця планування експеременту" + "-" * 70)
        print(Style.RESET_ALL)
        ...
        print('\t')
        print(Fore.BLUE + "\tКритерій Кохрена")
        print(Style.RESET_ALL)
        Gt = Critical_values.get_cohren_value(f2, f1, q)
        if Gt > Gp and m < 25:
            print(Fore.GREEN +
                  "\t\tДисперсія однорідна при рівні значимості {:.2f}!\n\tЗбільшувати m не потрібно.".format(q))
            print(Style.RESET_ALL)
            homogeneity = True
        else:
            print(Fore.RED + "\t\tДисперсія не однорідна при рівні значимості {:.2f}!".format(q))
            print(Style.RESET_ALL)
            m += 1
        if m == 25:
            exit()
    ...
    print(Fore.BLUE + "\tОтримане рівняння регресії з урахуванням критерія Стьюдента")
    print(Style.RESET_ALL)
    print("{:.3f} + {:.3f} * X1 + {:.3f} * X2 + {:.3f} * X3 + {:.3f} * Х1X2 + {:.3f} * Х1X3 + {:.3f} * Х2X3"
          "+ {:.3f} * Х1Х2X3 + {:.3f} * X11^2 + {:.3f} * X22^2 + {:.3f} * X33^2 = ŷ\n\tПеревірка"
    ...
    if fisher_test():
        print(Fore.GREEN + "\t\tРівняння регресії адекватне стосовно оригіналу")
        adequacy = True
    else:
